...
EOF
  ...
  hocon -f $webserver_conf set webserver.ssl-cert $ssl_cert
  hocon -f $webserver_conf set webserver.ssl-key $ssl_key
  hocon -f $webserver_conf set webserver.ssl-ca-cert $ssl_ca_cert
  hocon -f $webserver_conf set webserver.ssl-crl-path $ssl_crl_path
  ...
  hocon -f /etc/puppetlabs/puppetserver/conf.d/ca.conf set certificate-authority.allow-subject-alt-names "${CA_ALLOW_SUBJECT_ALT_NAMES}"
  ...
  if [ ! -f "$new_cadir/ca_crt.pem" ] && [ ! -f "$SSLDIR/ca/ca_crt.pem" ]; then
    ...
    if [ -n "$DNS_ALT_NAMES" ]; then
      current="$(puppet config print dns_alt_names --section main)"
      # shell parameter expansion to remove trailing comma if there is one
      updated="${DNS_ALT_NAMES%,}"
      if [ -n "$current" ]; then
        updated="$current","$updated"
      fi
      puppet config set dns_alt_names "$updated" --section main
    fi
    ...
    puppetserver ca setup --ca-name "${ca_name}" --config /etc/puppetlabs/puppet/puppet.conf
  fi
fi